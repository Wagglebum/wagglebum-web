// Wagglebum — React component library (TSX)
// Drop these into app/components/ in a Next.js 14 + Tailwind project.
// Requires: tailwind config with tokens from tailwind-tokens.js merged in.
// Relies on globals.css (in this handoff package) for font imports + base styles.
...
export function Tag({ children, tone = 'neutral' }: { children: ReactNode; tone?: 'neutral' | 'dark' | 'blush' | 'ok' }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-extrabold uppercase tracking-wider ${tones[tone]}`}>{children}</span>;
}
      ...
      <div className="ml-3 flex gap-1">
        {items.map(i =>
          <Link key={i.id} href={i.href}
            className={`rounded-full px-3.5 py-2 text-sm font-semibold no-underline ${current === i.id ? 'text-ink' : 'text-ink hover:bg-bone'}`}>
            {i.label}
          </Link>
        )}
      </div>
...
export function Hero({
  ...
  return (
    <section className="mx-auto max-w-[960px] px-6 pt-20 pb-10 text-center">
      {eyebrow && <Tag tone="blush"><span className="h-1.5 w-1.5 rounded-full bg-blush-500" />{eyebrow}</Tag>}
      <h1 className="mt-6 text-[clamp(48px,7vw,88px)] font-black leading-[1.02] tracking-[-0.025em]">{title}</h1>
      <p className="mx-auto mt-5 max-w-[620px] text-[19px] leading-relaxed text-fg-muted">{body}</p>
      ...
      {mascot && (
        <div className="mt-14 flex justify-center">
          <Image src="/brand/logo.png" alt="Wagglebum mascot" width={240} height={240} priority />
        </div>
      )}
    </section>
  );
...
export function Footer() {
  const cols: [string, { label: string; href: string }[]][] = [
    ['Product', [{ label: 'Games', href: '/games' }, { label: 'Plugins', href: '/plugins' }, { label: 'Services', href: '/services' }, { label: 'Changelog', href: '/changelog' }]],
    ['Company', [{ label: 'About', href: '/about' }, { label: 'Blog', href: '/blog' }, { label: 'Press', href: '/press' }]],
    ['Community', [{ label: 'Discord', href: '/discord' }, { label: 'GitHub', href: '/github' }, { label: 'Newsletter', href: '/newsletter' }]],
  ];
  return (
      ...
      <div className="mx-auto grid max-w-[1100px] grid-cols-1 gap-10 md:grid-cols-[2fr_1fr_1fr_1fr]">
        <div>
          ...
          <p className="max-w-[320px] text-sm leading-relaxed text-mist">
            Indie games and the tools to build them.
          </p>
        </div>
        {cols.map(([heading, items]) =>
          <div key={heading}>
            ...
            <div className="flex flex-col gap-2.5">
              {items.map(i => <Link key={i.label} href={i.href} className="text-sm font-medium text-snow no-underline">{i.label}</Link>)}
            </div>
          </div>
        )}
      </div>
